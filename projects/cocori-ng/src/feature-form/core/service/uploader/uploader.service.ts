...
export class UploaderService {
  ...
  private handlePartsFile(fileArrayBuffer: ArrayBuffer, numberParts: number) {
    ...
    this.sendFull(this.getPart(fileArrayBuffer, 0), numberParts).subscribe((datas: any) => {
      const fileId: string = datas.value
  ...
  private sendFile(command: NewFileCommand | FilePartCommand, partIndex: number, numberParts: number) {
    return this.UploadFileAPI(command, partIndex, numberParts).pipe(
  ...
  UploadFileAPI(datas: NewFileCommand | FilePartCommand, partIndex: number, numberParts: number): Observable<any> {
    let apiFile: string = this.apisFile.apiFile()

    if (numberParts > 1 && partIndex > 1) apiFile = `${apiFile}/${(<FilePartCommand>datas).fileId}/parts`
