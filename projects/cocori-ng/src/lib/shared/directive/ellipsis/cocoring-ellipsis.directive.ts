...
export class CocoringEllipsisDirective implements OnChanges, OnDestroy, AfterViewInit {
     ...
     */
    @Input('ellipsis-content') ellipsisContent!: string | number;
    ...
    @Input('ellipsis-resize-detection')
    ...
    public applyEllipsis() {
        ...
        if (this.changeEmitter.observers.length > 0) {
            this.changeEmitter.emit(
                (this.originalText.length === finalLength) ? 0 : finalLength
            );
        }
    }
